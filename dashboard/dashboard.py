...
# Daily Orders (Per Hari)
st.subheader("Daily Orders📆")

daily_orders_df = (main_hour_df.groupby("dateday", as_index=False)["count"].sum()
                   )

fig_daily = go.Figure()

fig_daily.add_trace(go.Scatter(
    x=daily_orders_df["dateday"],
    y=daily_orders_df["count"],
    mode="lines+markers",
    line=dict(width=2),
    marker=dict(size=6),
    name="Total Orders"
...
# Scatter Plot:
st.markdown("---")
st.subheader("Korelasi Suhu dengan Total Penyewaan🌡️🚲")

scatter_df = (
    main_hour_df
    .groupby(["dateday", "season_label"], as_index=False)
    .agg({
        "temp": "mean",
        "count": "sum"
    })
...
if scatter_df.empty:
    ...
    for season in scatter_df["season_label"].unique():
        season_df = scatter_df[scatter_df["season_label"] == season]
    ...
    "Winter": "#8D5580"    
        ...
        fig_scatter.add_trace(go.Scatter(
            ...
            marker=dict(
                size=9,
                color=season_colors.get(season, "#B0BEC5"),
                opacity=0.8,
                line=dict(width=1)
            ),
            hovertemplate=
            "%{text}<br>" +
            "Temp: %{x:.2f}<br>" +
            "Total Sewa: %{y:,}<extra></extra>",
            text=[season] * len(season_df)
        ))

    fig_scatter.update_layout(
        title="Korelasi Suhu (Temp) dengan Total Penyewaan Sepeda",
        xaxis_title="Temperatur (Normalized)",
...
st.write("Scatter rows:", len(scatter_df))
...
# Agregasi data
holiday_hourly = (
    main_hour_df[main_hour_df['workingday'] == 0].groupby("hour")[["casual", "registered"]].mean().reset_index()
)

workday_hourly = (
    main_hour_df[main_hour_df['workingday'] == 1].groupby("hour")[["casual", "registered"]].mean().reset_index()
)
